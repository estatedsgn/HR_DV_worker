...
class Dialog(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    crmchat_dialog_id: Mapped[str] = mapped_column(
        String(255), unique=True, index=True, nullable=False
    )
    lead_external_id: Mapped[str | None] = mapped_column(String(255), index=True)
    telegram_peer_type: Mapped[str | None] = mapped_column(String(50), index=True)
    telegram_peer_id: Mapped[str | None] = mapped_column(String(255), index=True)
    telegram_access_hash: Mapped[str | None] = mapped_column(String(255))
    ...
    outbound_logs: Mapped[list["OutboundSendLog"]] = relationship(
        back_populates="dialog", cascade="all, delete-orphan"
    )
    handoffs: Mapped[list["HumanHandoff"]] = relationship(
        back_populates="dialog", cascade="all, delete-orphan"
    )
