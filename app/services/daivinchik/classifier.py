...
# Кнопки, которыми закрывают/пропускают блокирующие экраны Дайвинчика
# (советы по безопасности, реклама, опросы, премиум-апселл). По правилу —
# жмём правую кнопку; эти подписи распознаём явно, чтобы не промахнуться.
DISMISS_MARKERS = (
    "не интересно",
    "не сейчас",
    "пропустить",
...
def classify(
    ...
    *,
    ...
    # 7) Любой неузнанный экран С КНОПКАМИ — это блокирующий интерстициал, который
    # Дайвинчик вставляет между анкетами (совет по безопасности, реклама, опрос,
    # премиум-апселл). Если просто молчать — свайпер виснет на нём навсегда. Поэтому
    # пробиваем его: жмём кнопку «пропустить/не интересно/дальше», а по умолчанию —
    # самую правую (таково правило для этих экранов). Так свайпер всегда движется.
    if message.has_buttons:
        target = _find_button_by_text(message, DISMISS_MARKERS) or message.button_from_right(1)
        if target is not None:
            return Decision(
                intent="dismiss",
                press=target,
                review=True,  # пишем образец, чтобы потом доучить классификатор
                note=f"unrecognized interstitial -> dismiss ('{target.text}')",
            )

    # 8) Неузнанное и без кнопок (пустое медиа и т.п.) -> в ревью, ждём.
    return Decision(
        intent="unknown",
        review=True,
